...
If !Empty(aNota)
	...
	///////////////////////////////////////////////////
	/// Rotta - 06/01/2022
	///       Tratamento para aglutinação dos lotes 
	///       e impressão única por código de produto
    cAjusteProd := .F.//Rotta - 06/01/2022
	If aDest[9] <> "EX"
		aProd2     := aProd
		aICMS2     := aICMS
		aICMSST2   := aICMSST
		...
		aPISST2    := aPISST
		aCOFINS2   := aCOFINS
		aCOFINSST2 := aCOFINSST
		aISSQN2    := aISSQN
		aCST2      := aCST
		aMed2      := aMed
		aArma2     := aArma
		aveicProd2 := aveicProd
		aDI2       := aDI
		aAdi2      := aAdi
		aExp2      := aExp
		aPisAlqZ2  := aPisAlqZ
		...
		aCsosn2    := aCsosn
		aPedCom2   := aPedCom
		aIPIDevol2 := aIPIDevol
		aICMUFDes2 := aICMUFDest
		aProd     := {}
		aICMS     := {}
		aICMSST   := {}
		aIPI      := {}
		...
		For nX := 1 To Len(aProd2)
			nAchouProd := aScan(aProd,{|x| x[2] == aProd2[nX,2]})
			If nAchouProd > 0 .and.  cD2Tes >= "500" .and. Substr(aProd2[nX,2],1,2) <> "02"
				cAjusteProd := .T.
				aProd[nAchouProd,09] += aProd2[nX,09]  // Quantidade
				aProd[nAchouProd,10] += aProd2[nX,10]  // Valor Total
				aProd[nAchouProd,16] := (aProd[nAchouProd,10] / aProd[nAchouProd,09])  // Valor Unitario
				aProd[nAchouProd,12] += aProd2[nX,12]
				aProd[nAchouProd,13] += aProd2[nX,13]
				aProd[nAchouProd,14] += aProd2[nX,14]
				aProd[nAchouProd,15] += aProd2[nX,15]
				aProd[nAchouProd,26] += aProd2[nX,26]
				aProd[nAchouProd,22] += aProd2[nX,22]
				aProd[nAchouProd,35] += aProd2[nX,35]
				aProd[nAchouProd,36] += aProd2[nX,36]
				aProd[nAchouProd,37] += aProd2[nX,37]
				aProd[nAchouProd,43] += aProd2[nX,43]
				If Len(aICMS2[nX]) > 0
					aICMS[nAchouProd,05] += aICMS2[nX,05]
					aICMS[nAchouProd,07] += aICMS2[nX,07]
					aICMS[nAchouProd,09] += aICMS2[nX,09]
					If aICMS2[nX,07] > 0
						aICMS[nAchouProd,11] := aICMS2[nX,11]
					Endif
					aICMS[nAchouProd,12] += aICMS2[nX,12]
					aICMS[nAchouProd,15] += aICMS2[nX,15]
					aICMS[nAchouProd,16] += aICMS2[nX,16]
					aICMS[nAchouProd,18] += aICMS2[nX,18]
				Endif
				If Len(aICMSST2[nX]) > 0
					aICMSST[nAchouProd,05] += aICMSST2[nX,05]
					aICMSST[nAchouProd,07] += aICMSST2[nX,07]
					aICMSST[nAchouProd,12] += aICMSST2[nX,12]
					aICMSST[nAchouProd,13] += aICMSST2[nX,13]
					aICMSST[nAchouProd,15] += aICMSST2[nX,15]
				Endif
				If Len(aIPI2[nX]) > 0
					If Len(aIPI[nAchouProd]) > 0
						aIPI[nAchouProd,06] += aIPI2[nX,06]
						aIPI[nAchouProd,07] += aIPI2[nX,07]
						aIPI[nAchouProd,10] += aIPI2[nX,10]
					Endif
				Endif
				If Len(aPIS2[nX]) > 0
					aPIS[nAchouProd,02] += aPIS2[nX,02]
					aPIS[nAchouProd,04] += aPIS2[nX,04]
				Endif
				If Len(aPISST2[nX]) > 0
					aPISST[nAchouProd,02] += aPISST2[nX,02]
					aPISST[nAchouProd,04] += aPISST2[nX,04]
				Endif
				If Len(aCOFINS2[nX]) > 0
					aCOFINS[nAchouProd,02] += aCOFINS2[nX,02]
					aCOFINS[nAchouProd,04] += aCOFINS2[nX,04]
				Endif
				If Len(aCOFINSST2[nX]) > 0
					aCOFINSST[nAchouProd,02] += aCOFINSST2[nX,02]
					aCOFINSST[nAchouProd,04] += aCOFINSST2[nX,04]
				Endif
				If Len(aISSQN2[nX]) > 0
					aISSQN[nAchouProd,02] += aISSQN2[nX,02]
					aISSQN[nAchouProd,05] += aISSQN2[nX,05]
					aISSQN[nAchouProd,06] += aISSQN2[nX,06]
				Endif
				If Len(aMed2[nX]) > 0
					aMed[nAchouProd,02] += aMed2[nX,02]
					aMed[nAchouProd,05] += aMed2[nX,05]
				Endif
				If Len(aAnfI2[nX]) > 0
					aAnfI[nAchouProd,10] += aAnfI2[nX,10]
					aAnfI[nAchouProd,11] += aAnfI2[nX,11]
					aAnfI[nAchouProd,15] += aAnfI2[nX,15]
					aAnfI[nAchouProd,16] += aAnfI2[nX,16]
				Endif
				If Len(aComb2[nX]) > 0
					aComb[nAchouProd,03] += aComb2[nX,03]
					If Len(aComb2[nX]) > 3
						aComb[nAchouProd,05] += aComb2[nX,05]
						aComb[nAchouProd,06] += aComb2[nX,06]
						aComb[nAchouProd,07] += aComb2[nX,07]
					Endif
				Endif
				If Len(aIPIDevol2[nX]) > 0
					aIPIDevol[nAchouProd,02] += aIPIDevol2[nX,02]
				Endif
				If Len(aICMUFDes2[nX]) > 0
					aICMUFDest[nAchouProd,01] += aICMUFDes2[nX,01]
					aICMUFDest[nAchouProd,06] += aICMUFDes2[nX,06]
					aICMUFDest[nAchouProd,07] += aICMUFDes2[nX,07]
					aICMUFDest[nAchouProd,08] += aICMUFDes2[nX,08]
				Endif
			Else
				aadd(aProd, aProd2[nX])
				aadd(aICMS, aICMS2[nX])
				aadd(aICMSST, aICMSST2[nX])
				aadd(aIPI, aIPI2[nX])
				aadd(aPIS, aPIS2[nX])
				aadd(aPISST, aPISST2[nX])
				aadd(aCOFINS, aCOFINS2[nX])
				aadd(aCOFINSST, aCOFINSST2[nX])
				aadd(aISSQN, aISSQN2[nX])
				aadd(aCST, aCST2[nX])
				aadd(aMed, aMed2[nX])
				aadd(aArma, aArma2[nX])
				aadd(aveicProd, aveicProd2[nX])
				aadd(aDI, aDI2[nX])
				aadd(aAdi, aAdi2[nX])
				aadd(aExp, aExp2[nX])
				aadd(aPisAlqZ, aPisAlqZ2[nX])
				aadd(aCofAlqZ, aCofAlqZ2[nX])
				aadd(aAnfI, aAnfI2[nX])
				aadd(aComb, aComb2[nX])
				aadd(aCsosn, aCsosn2[nX])
				aadd(aPedCom, aPedCom2[nX])
				aadd(aIPIDevol, aIPIDevol2[nX])
				aadd(aICMUFDest, aICMUFDes2[nX])
			EndIf
		Next nX
	Endif
	If cAjusteProd   // Ajusta seq.do item
		For nX := 1 To Len(aProd)
			aProd[nX,01] := nX
		Next nX
	Endif
	/// Rotta - 06/01/2022 - Fim da implementação para aglutinação dos lotes ////////////////////////

	/// Rotta - 06/01/2022 - Tratamento de quantidade e impressão lote ////////////////////////
	For nX := 1 To Len(aProd)
		cProd   := aProd[nX][2]
		cBar    := aProd[nX][3]
		cUM     := aProd[nX][8]
		nQuantItem := aProd[nX][9]
		nVolume += aProd[nX][9]
		DbSelectArea("SB1")
		dbSetOrder(1)
		dbSeek(xFilial()+cProd)
		If SB1->B1_TIPO == "PA"
			If nQuantItem-Int(nQuantItem) > 0 .and. cUM <> "KG"
				nQtdSeg := Round(ConvUm(cProd,nQuantItem,0,2),2)
				c2UM := SB1->B1_SEGUM
				If nQtdSeg > 0 .and. (nQtdSeg-Int(nQtdSeg)) == 0 .and. !Empty(c2UM)
					aProd[nX][8] := c2UM
					aProd[nX][9] := nQtdSeg
					aProd[nX][11] := c2UM
					aProd[nX][12] := nQtdSeg
					aProd[nX][16] := Round(aProd[nX,10]/aProd[nX,9],2)
				Endif
			Endif
			If Substr(cProd,1,2) == "02"
				If !Empty(aProd[nX][19])
					aProd[nX][4] := Alltrim(aProd[nX][4]) + " Lote: " + Alltrim(aProd[nX][19])
				Endif
			Endif
		Endif
	Next

	/// Rotta - 06/01/2022 - Fim Tratamento de quantidade e impressão lote ////////////////////////
